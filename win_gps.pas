unit win_gps;

interface

uses
  System.SysUtils, System.Classes, JS, Web, WEBLib.Graphics, WEBLib.Controls,
  WEBLib.Forms, WEBLib.Miletus, WEBLib.Dialogs, gps, WEBLib.USB,
  Vcl.Controls, Vcl.StdCtrls, WEBLib.StdCtrls;

type
  TfrmWinGPS = class(TfrmGPS)
    WebUSBSerial1: TWebUSBSerial;
    procedure lblStatusClick(Sender: TObject);
    procedure WebUSBSerial1OpenPort(Sender: TObject; APort: TSerialPort);
    procedure WebUSBSerial1ReadData(Sender: TObject; AData: TJSUint8Array;
      ADone: Boolean);
    procedure WebUSBSerial1Connected(Sender: TObject; APort: TSerialPort);
    procedure lblPositionClick(Sender: TObject);
  private
    { Private declarations }
    procedure OpenPort;
  public
    { Public declarations }
  end;

var
  frmWinGPS: TfrmWinGPS;

implementation

{$R *.dfm}

procedure TfrmWinGPS.lblPositionClick(Sender: TObject);
begin
    WebUSBSerial1.SelectedPort := WebUSBSerial1.Ports.Items[0];

    WebUSBSerial1.OpenPort;
end;

procedure TfrmWinGPS.lblStatusClick(Sender: TObject);
begin
    OpenPort;
end;

procedure TfrmWinGPS.OpenPort;
begin
    if WebUSBSerial1.HasSerial then begin
        lblStatus.Caption := 'Requesting Port';
        WebUSBSerial1.RequestPorts;

        lblStatus.Caption := 'Initialising Port';
        WebUSBSerial1.InitPorts;
    end;
end;

procedure TfrmWinGPS.WebUSBSerial1Connected(Sender: TObject;
  APort: TSerialPort);
begin
    lblStatus.Caption := 'Connected OK';
end;

procedure TfrmWinGPS.WebUSBSerial1OpenPort(Sender: TObject;
  APort: TSerialPort);
begin
    WebUSBSerial1.Read;
end;

procedure TfrmWinGPS.WebUSBSerial1ReadData(Sender: TObject;
  AData: TJSUint8Array; ADone: Boolean);
const
    Line: String = '';
var
    i: Integer;
begin
    lblStatus.Caption := '';

    for i := 1 to AData.length do begin
        if Adata.values[i-1] = 10 then begin
            ProcessLine(Line);
            Line := '';
        end else if Chr(Adata.values[i-1]) = '$' then begin
            Line := '$';
        end else if Line <> '' then begin
            Line := Line + Chr(Adata.values[i-1]);
        end;
    end;

    WebUSBSerial1.Read;
end;

initialization
  RegisterClass(TfrmWinGPS);


end.